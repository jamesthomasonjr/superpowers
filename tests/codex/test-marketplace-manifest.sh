...
assert_equal(marketplace.get("name"), "supersuit-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "supersuit"]
assert_equal(len(matching_plugins), 1, "supersuit plugin entry count")
